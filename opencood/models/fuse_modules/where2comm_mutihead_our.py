...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, query, key, value, source_confidence=None, bias_strength=1.0):
        score = torch.bmm(query, key.transpose(1, 2)) / self.sqrt_dim
        if source_confidence is not None:
            # source_confidence shape: [cav_num]
            conf = source_confidence.to(score.device).float()
            conf = torch.clamp(conf, min=1e-4, max=1.0)

            # log(conf) <= 0. Reliable CAV gets near 0 penalty.
            bias = bias_strength * torch.log(conf)

            # apply bias along key/source dimension
            # score shape: [H*W, cav_num, cav_num]
...
class AttenFusion(nn.Module):
    ...
    def forward(self, x, uncertainty_confidence=None, uncertainty_attention_bias_strength=1.0):
        ...
        x = self.danet(
            ...
            uncertainty_attention_bias_strength=uncertainty_attention_bias_strength
        )[0]
...
class PositionAttentionModule(nn.Module):
    ...
    def forward(self,x,uncertainty_confidence=None,uncertainty_attention_bias_strength=1.0):
        ...
        y = self.pa(
            ...
            y,
            y,
            source_confidence=uncertainty_confidence,
            bias_strength=uncertainty_attention_bias_strength
        )
...
class DAModule(nn.Module):
    ...
    def forward(self,input,uncertainty_confidence=None, uncertainty_attention_bias_strength=1.0):
        ...
        p_out = self.position_attention_module(
            input,
            uncertainty_confidence=uncertainty_confidence,
            uncertainty_attention_bias_strength=uncertainty_attention_bias_strength
        )
...
class Where2comm(nn.Module):
    ...
    def forward(self, x, record_len, backbone=None, heads=None, uncertainty_confidence=None, uncertainty_attention_bias_strength=1.0):
        """
        Fusion forwarding.
        ...
        if self.multi_scale:
            ...
            for i in range(self.num_levels):
                ...
                if uncertainty_confidence is not None:
                    batch_uncertainty_confidence = self.regroup(uncertainty_confidence, record_len)
                else:
                    batch_uncertainty_confidence = None
                ...
                for b in range(B):
                    ...
                    if batch_uncertainty_confidence is not None:
                        neighbor_confidence = batch_uncertainty_confidence[b]
                    else:
