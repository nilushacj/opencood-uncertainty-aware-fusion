...
class PointPillarWhere2commOur(nn.Module):
    ...
    def directional_blur(self,
            features_2d: torch.Tensor,
            Sigma_params: np.ndarray,
            sigma_scale: float  = 1.0,
            sigma_min:   float  = 0.0,
            sigma_max:   float  = 3.0,
            apply_threshold: float = 0.1,
        ) -> tuple[torch.Tensor, bool, dict]:
        """
        Blurs features anisotropically using the eigenvectors of the 2-D
        translation covariance sub-matrix Sigma[1:3, 1:3].
    
        Steps:
        1. Eigen-decompose Sigma_tx_ty -> eigenvalues lam, eigenvectors V
        2. sigma_i = sqrt(lam_i) * sigma_scale, clamped to [sigma_min, sigma_max]
        3. For each principal direction v_i, rotate the feature map so that v_i
            aligns with the Y axis, apply a 1-D vertical Gaussian with sigma_i,
            then rotate back.
        4. Results from both directions are averaged.
        ...
        if isinstance(Sigma_params, torch.Tensor):
            Sigma_params = Sigma_params.detach().cpu().numpy()

        Sigma_tx_ty = np.asarray(Sigma_params, dtype=np.float64)[1:3, 1:3]
        eigenvalues, eigenvectors = np.linalg.eigh(Sigma_tx_ty)  # ascending order
        ...
        for i in range(2):
            lam  = max(float(eigenvalues[i]), 0.0)
            sig  = math.sqrt(lam) * sigma_scale
            sig  = float(np.clip(sig, sigma_min, sigma_max))
            sigmas_used.append(sig)
    
            if sig < apply_threshold:
                blurred_outputs.append(features_2d)
                continue
    
            # Angle of this eigenvector w.r.t. x-axis
            v     = eigenvectors[:, i]        # [vx, vy]
            angle = math.atan2(float(v[1]), float(v[0]))  # radians
    
            # Build affine rotation that maps v -> Y axis
            # theta_rot makes v align with [0,1], i.e. rotate by -(angle - pi/2)
            theta_rot = -(angle - math.pi / 2)
            cos_r, sin_r = math.cos(theta_rot), math.sin(theta_rot)
    
            # Normalised rotation matrix [1,2,3]
            rot_norm = np.float32([[cos_r, -sin_r, 0.0],
                                [sin_r,  cos_r, 0.0]])
            rot_inv  = np.float32([[cos_r,  sin_r, 0.0],
                                [-sin_r, cos_r, 0.0]])
    
            rot_t   = torch.from_numpy(rot_norm).to(device).unsqueeze(0)
            unrot_t = torch.from_numpy(rot_inv).to(device).unsqueeze(0)
    
            # Rotate feature map
            grid_rot  = F.affine_grid(rot_t,   [B, C, H, W], align_corners=False)
            rotated   = F.grid_sample(features_2d, grid_rot, align_corners=False)
    
            # 1-D vertical Gaussian blur
            radius  = max(1, int(math.ceil(3.0 * sig)))
            ksize   = 2 * radius + 1
            coords  = torch.arange(-radius, radius + 1, device=device, dtype=dtype)
            k1d     = torch.exp(-(coords ** 2) / (2 * sig ** 2))
            k1d     = k1d / k1d.sum()
            k2d     = k1d.view(1, 1, ksize, 1).repeat(C, 1, 1, 1)
    
            blurred_rot = F.conv2d(rotated, k2d, bias=None,
                                stride=1, padding=(ksize // 2, 0), groups=C)
    
            # Rotate back
            grid_unrot = F.affine_grid(unrot_t, [B, C, H, W], align_corners=False)
            unrotated  = F.grid_sample(blurred_rot, grid_unrot, align_corners=False)
            blurred_outputs.append(unrotated)
    
        result = torch.stack(blurred_outputs, dim=0).mean(dim=0)
    
        stats = {
            'method': 'anisotropic_directional_blur',
            'sigma_axis0': sigmas_used[0],
            'sigma_axis1': sigmas_used[1],
            'applied': any(s >= apply_threshold for s in sigmas_used),
        }
        return result, stats['applied'], stats

    def __params_to_affine(self, theta: float, tx: float, ty: float) -> np.ndarray:
        """[theta, tx, ty] -> 2x3 affine (pixel coords)."""
        c, s = math.cos(theta), math.sin(theta)
        return np.float32([[c, -s, tx], [s, c, ty]])

    def __trans_tx(self, t_matrix: np.ndarray, H: int, W: int) -> np.ndarray:
        """Pixel-space 3x3 affine -> normalised 2x3 for F.affine_grid."""
        a00, a01 = t_matrix[0][0], t_matrix[0][1]
        a02, a12 = t_matrix[0][2], t_matrix[1][2]
        tx = (a02 - ((1 - a00) * W * 0.5 - a01 * H * 0.5)) / (W * 0.5)
        ty = (a12 - (a01 * W * 0.5 + (1 - a00) * H * 0.5)) / (H * 0.5)
        T = np.float32([[a00, a01, tx], [a01, a00, ty], [0, 0, 1]])
        if np.linalg.det(T) == 0:
            return np.float32([[1, 0, 0], [0, 1, 0]])
        return np.linalg.inv(T)[0:2, :]

    def __warp(self, src: torch.Tensor, t_matrix: torch.Tensor, H: int, W: int) -> torch.Tensor:
        """src: [1,C,H,W], t_matrix: [1,2,3] -> [1,C,H,W]."""
        grid = F.affine_grid(t_matrix, [1, src.shape[1], H, W], align_corners=False)
        return F.grid_sample(src, grid, align_corners=False)

    def __build_t_matrix(self, theta, tx, ty, mask_h, mask_w, device) -> torch.Tensor:
        affine = self.__params_to_affine(float(theta), float(tx), float(ty))
        norm   = self.__trans_tx(affine, mask_h, mask_w)
        return torch.from_numpy(norm).to(device).unsqueeze(0)   # [1,2,3]

    def covariance_weighted_mc_warp(self,
            features_2d: torch.Tensor,   # [C, H, W]
            mu_params:   np.ndarray,     # [theta, tx, ty]
            Sigma_params: np.ndarray,    # [3,3]
            mask_h: int, mask_w: int,
            H: int, W: int,
            num_samples: int = 5,
            theta_clip: float = 0.15,
            translation_clip: float = 3.0,
        ) -> tuple[torch.Tensor, bool, dict]:
        """
        MC warp where each sample is weighted by its Gaussian likelihood.
    
        w_k = exp(-0.5 * d_k^2)  where  d_k = Mahalanobis(sample_k, mu, Sigma)
        The weights are normalised to sum to 1 before accumulation.
    
        Returns:
            warped_weighted: [1, C, H, W]
            applied: bool
            stats: dict
        """
        if num_samples <= 0:
        ...
        device = features_2d.device
        # x      = features_2d.unsqueeze(0)  # [1, C, H, W]
        x = features_2d if features_2d.dim() == 4 else features_2d.unsqueeze(0)
    
        warped_list  = []
        log_weights  = []
    
        for _ in range(num_samples):
            sample = np.random.multivariate_normal(mu, Sigma_reg)
    
            # Safety clip
            delta        = sample - mu
            delta[0]     = np.clip(delta[0], -theta_clip,      theta_clip)
            delta[1:]    = np.clip(delta[1:], -translation_clip, translation_clip)
            sample       = mu + delta
    
            # Mahalanobis distance -> log-weight
            maha2        = float(delta @ Sigma_inv @ delta)
            log_weights.append(-0.5 * maha2)
    
            t_mat        = self.__build_t_matrix(sample[0], sample[1], sample[2], mask_h, mask_w, device)
            warped_list.append(self.__warp(x, t_mat, H, W))   # [1,C,H,W]
    
        # Normalise weights in log-space for numerical stability
        ...
        # Weighted sum
        warped_stack = torch.stack(warped_list, dim=0)                   # [K,1,C,H,W]
        w_tensor     = torch.tensor(w, dtype=warped_stack.dtype,
                                    device=device).view(-1, 1, 1, 1, 1)
        warped_out   = (warped_stack * w_tensor).sum(dim=0)              # [1,C,H,W]
    
        stats = {
            'method': 'covariance_weighted_mc',
            'num_samples': num_samples,
            'weight_entropy': float(-np.sum(w * np.log(w + 1e-12))),
            'effective_samples': float(1.0 / (w ** 2).sum()),
        }
        return warped_out, True, stats
    ...
    def forward(self, data_dict):
        ...
        for i in range(len(communication_masks)):
            ...
            for j in range(1,cav_num):
                ...
                if blur_is_allowed:
                    blur_sigma = self.covariance_to_blur_sigma(Sigma_params)
                ...
                # --- NOTE: Directional blur ---
                features_2d, blur_applied, blur_stats = self.directional_blur(
                    features_2d=features_2d, 
                    Sigma_params=Sigma_params.detach().cpu().numpy() if isinstance(Sigma_params, torch.Tensor) else Sigma_params,
                    sigma_scale=blur_sigma
                )

                # directional blur uses two directions, so we calculate max kernel size for the logger
                if blur_applied and isinstance(blur_stats, dict):
                    max_sig = max(blur_stats.get('sigma_axis0', 0.0), blur_stats.get('sigma_axis1', 0.0))
                    blur_kernel_size = int(2 * max(1, math.ceil(3.0 * max_sig)) + 1)
                else:
                    blur_kernel_size = 1

                # --- NOTE: Covariance weighted warp ---
                # features_2d, blur_applied, blur_stats = self.covariance_weighted_mc_warp(
                #     features_2d=features_2d,
                #     mu_params=mu_params,
                #     Sigma_params=Sigma_params,
                #     mask_h=mask_h,
