...
def get_t_matrix_with_stats(mask0, mask1):
    """
    Original FeaCo deterministic PRM transform estimation, with diagnostics.

    Input:
        mask0: ego communication mask
        mask1: collaborator communication mask

    Returns:
        t_matrix: np.float32 of shape (2, 3)
            Deterministic affine transform from mask1 to mask0.
    ...
    match_costs = np.array(
        [cost_matrix[i, j] for i, j in zip(index0, index1)],
        dtype=np.float32
    )

    stats['num_matches_before_mad'] = int(len(match_costs))
    stats['mean_match_cost_before_mad'] = (
        float(np.mean(match_costs)) if len(match_costs) > 0 else None
    )
    ...
    # ---------------------------------
    # 5) Original FeaCo ICP transform
    # ---------------------------------
    target_pts = centroids0_matched.T  # shape (2, N)
    source_pts = centroids1_matched.T  # shape (2, N)

    R, T, thera, A_mean, B_mean = ICP_2D(target_pts, source_pts)
    ...
    # ---------------------------------
    aligned_source = (R @ source_pts) + T
    residuals = target_pts - aligned_source
    residual_norms = np.linalg.norm(residuals, axis=0)

    stats['icp_residual_mean'] = (
        float(np.mean(residual_norms)) if residual_norms.size > 0 else None
    )
    stats['icp_residual_std'] = (
        float(np.std(residual_norms)) if residual_norms.size > 0 else None
    )

    stats['status'] = 'ok'

    return t_matrix, stats
