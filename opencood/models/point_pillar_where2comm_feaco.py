...
from opencood.models.sub_modules.positioning_error_correction import get_t_matrix, get_t_matrix_with_stats
import torch
import os
import csv
...
class PointPillarWhere2commFeaco(nn.Module):
    def __init__(self, args):
        super(PointPillarWhere2commFeaco, self).__init__()
    ...
    def forward(self, data_dict):
        ...
        for i in range(len(communication_masks)):
            ...
            for j in range(1,cav_num):
                ...
                t_matrix, stats = get_t_matrix_with_stats(ego_mask,other_mask) # get fine-grid transformation matrix
                theta = float(np.arctan2(t_matrix[1, 0], t_matrix[0, 0]))
                tx = float(t_matrix[0, 2])
                ty = float(t_matrix[1, 2])

                # # ---------- DEBUG SAVE BLOCK ----------
                # if self.debug_save_count < 25:
                #     debug_dir = "debug_prm"
                #     os.makedirs(debug_dir, exist_ok=True)

                #     np.save(os.path.join(debug_dir, f"ego_mask_{self.debug_save_count}.npy"), ego_mask)
                #     np.save(os.path.join(debug_dir, f"other_mask_{self.debug_save_count}.npy"), other_mask)
                #     np.save(os.path.join(debug_dir, f"t_matrix_raw_{self.debug_save_count}.npy"), t_matrix)

                #     print(f"[DEBUG] Saved sample {self.debug_save_count}")
